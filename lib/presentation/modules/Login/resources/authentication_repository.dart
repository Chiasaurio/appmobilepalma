...
class AuthenticationRepository {
  ...
  Future<int> logIn(String ccUsuario, String password) async {
    try {
      final body = jsonEncode({'cc_usuario': ccUsuario, 'contrasena_usuario': password});
      final res = await _dio.post(
        '${BaseApi.baseUrl}login',
        data: body,
        options: Options(headers: {HttpHeaders.contentTypeHeader: 'application/json'}, responseType: ResponseType.json),
      );

      final respDecode = res.data;
      if (res.statusCode == 200) {
        globals.responsable = ccUsuario;
        await setUserCredentials(respDecode['token'], ccUsuario, password, password);
