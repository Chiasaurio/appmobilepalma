      ...
      body: BlocListener<LoginCubit, LoginState>(
        listener: (context, state) {
              ...
              ..showSnackBar(
                const SnackBar(
                  content: Row(
                    ...
                    children: [Text('Iniciando sesión'), CircularProgressIndicator()],
                  ),
                ),
              );
          }
          if (state.status == FormzSubmissionStatus.failure) {
            ScaffoldMessenger.of(context)
              ..hideCurrentSnackBar()
              ..showSnackBar(const SnackBar(backgroundColor: Colors.red, content: Text('Error al iniciar sesión')));
          }
          if (state.status == FormzSubmissionStatus.success) {
            Navigator.of(context).pushNamedAndRemoveUntil('/finca', (_) => false);
          }
        },
        child: Padding(
                    ...
                    return MainButtonSquare(
                      ...
                      press: () {
                        if (state.status == FormzSubmissionStatus.initial ||
                          ...
                          BlocProvider.of<LoginCubit>(context).signUpFormSubmitted();
                        }
                      },
                    );
                  },
