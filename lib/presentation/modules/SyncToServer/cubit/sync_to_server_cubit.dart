...
class SyncToServerCubit extends Cubit<SyncToServerState> {
  ...
  void getRegistrosPendientes() async {
    ...
    final fertilizacionesPendientes = await getFertlizacionesPendientesSincronizar();
    ...
    final erradicacionesPendientes = await getErradicacionesPendientesSincronizar();
    ...
    final censosProductivosPendientes = await getCensosProductivosPendientesSync();
  ...
  Future<List<PrecipitacionData>> getPrecipitacionesPendientesSync() async {
    try {
      ...
      final precipitaciones = await loteDao.getPrecipitacionesPendientesForSync();
  ...
  Future<List<CensoProductivoData>> getCensosProductivosPendientesSync() async {
    try {
      ...
      final censosProductivos = await censoProductivoDao.getCensosPendientesForSync();
  ...
  Future<List<CosechaConCosechasDiarias>> getCosechasPendientesSincronizar() async {
    try {
      ...
      for (var c in cosechas) {
        final cosechasDiarias = await cosechaDiariaDao.getCosechasDiariasForSync(c!.id);
        cosechasConDiarias.add(CosechaConCosechasDiarias(cosecha: c, cosechasdiarias: cosechasDiarias));
      }
  ...
  Future<List<PodasConPodasDiarias>> getPodasPendientesSincronizar() async {
    try {
      ...
      for (var p in podas) {
        ...
        podasConDiarias.add(PodasConPodasDiarias(poda: p, podasDiarias: podasDiarias));
      }
  ...
  Future<List<PlateoConPlateosDiarias>> getPlateosPendientesSincronizar() async {
    try {
      ...
      for (var p in plateos) {
        ...
        plateoConDiarias.add(PlateoConPlateosDiarias(plateo: p, plateosDiarias: plateosDiarias));
      }
  ...
  Future<List<FertilizacionConFertilizacionesDiarias>> getFertlizacionesPendientesSincronizar() async {
    try {
      ...
      for (var f in fertilizaciones) {
        final fertilizacionesDiarias = await fertilizacionDao.getFertilizacionesDiariasForSync(f!.id);
        fertilizacionConDiarias.add(
          FertilizacionConFertilizacionesDiarias(fertilizacion: f, fertilizacionesDiarias: fertilizacionesDiarias),
        );
      }
  ...
  Future<List<RegistroEnfermedadConImagenes>> getEnfermedadesPendientesSincronizar() async {
    try {
      final PalmaDao palmasDao = db.palmaDao;
      final registrosEnfermedades = await palmasDao.getRegistrosEnfermedadesForSync();
      ...
      for (var r in registrosEnfermedades) {
        ...
        registroConImagenes.add(RegistroEnfermedadConImagenes(registroEnfermedad: r, imagenes: imagenes));
      }
  ...
  Future<List<RegistroTratamientoData>> getTratamientosPendientesSincronizar() async {
    try {
      final PalmaDao palmasDao = db.palmaDao;
      final registrosTratamientos = await palmasDao.getRegistrosTratamientosForSync();
  ...
  Future<List<ErradicacionData>> getErradicacionesPendientesSincronizar() async {
    try {
      final ErradicacionesDao erradicacionesDao = db.erradicacionesDao;
      final registroErradicaciones = await erradicacionesDao.getRegistrosForSync();
  ...
  Future<List<CensoConEtapas>> getCensosPendientesSincronizar() async {
    try {
      ...
      for (var r in registroCensos) {
        ...
        censoConEtapas.add(CensoConEtapas(censo: r, etapas: etapas, imagenes: imagenes));
      }
  ...
  Future<bool> syncCensosProductivos() async {
    try {
      ...
      final res = await remote.syncCensosProductivos(state.censosProductivosPendientes ?? []);
      if (res["success"]) {
        final CensoProductivoDao censoProductivoDao = db.censoProductivoDao;
        var index = 0;
        ...
        for (var censo in state.censosProductivosPendientes!) {
          if (ids.isNotEmpty && ids[index] > -1) {
            ...
            await censoProductivoDao.updateCenso(censo.copyWith(idCensoProductivo: Value(ids[index])));
          }
          index++;
  ...
  Future<bool> syncPrecipitaciones() async {
    try {
      ...
      final res = await remote.syncPrecipitaciones(state.precipitacionesPendientes ?? []);
      if (res["success"]) {
        final LoteDao loteDao = db.loteDao;
    ...
    try {
      ...
      if (res["success"]) {
        ...
        for (var viaje in state.viajesPendientes!) {
          if (ids[index] > -1) {
            //si agrego el viaje se actualiza el id
            await viajesDao.updateViaje(viaje.copyWith(idViaje: Value(ids[index])));
          }
          index++;
        }
    ...
    try {
      ...
      for (var registro in state.cosechasConDiariasPendientes!) {
        if (registro.cosecha.idViaje != null) {
          ...
          if (viaje != null) {
            await cosechaDao.updateCosecha(registro.cosecha.copyWith(idViajeFromServer: Value(viaje.idViaje!)));
            nuevosRegistrosDeCosechas.add(
              CosechaConCosechasDiarias(
                cosechasdiarias: registro.cosechasdiarias,
                cosecha: registro.cosecha.copyWith(idViajeFromServer: Value(viaje.idViaje!)),
              ),
            );
          }
        } else {
      ...
      final res = await remote.syncCosechasConDiarias(nuevosRegistrosDeCosechas);
      if (res["success"]) {
        var index = 0;
        ...
        for (var cosecha in state.cosechasConDiariasPendientes!) {
          if (ids[index] > -1) {
            ...
            await cosechaDao.updateCosecha(cosecha.cosecha.copyWith(idCosecha: Value(ids[index])));
          }
          index++;
        }
  ...
  Future<bool> syncPlateos() async {
    try {
      ...
      final res = await remote.syncPlateosConDiarias(state.plateosConDiariasPendientes ?? []);
      if (res["success"]) {
        final PlateoDao plateoDao = db.plateoDao;
        var index = 0;
        ...
        for (var plateo in state.plateosConDiariasPendientes!) {
          if (ids[index] > -1) {
            //si agrego la cosecha, toca actualizar el id
            await plateoDao.updatePlateo(plateo.plateo.copyWith(idPlateo: Value(ids[index])));
          }
          index++;
        }
  ...
  Future<bool> syncFertilizaciones() async {
    try {
      ...
      final res = await remote.syncFertilizacionesConDiarias(state.fertilizacionesConDiariasPendientes ?? []);
      if (res["success"]) {
        final FertilizacionDao fertilizacionDao = db.fertilizacionDao;
        var index = 0;
        ...
        for (var fertilizacion in state.fertilizacionesConDiariasPendientes!) {
          if (ids[index] > -1) {
            //si agrego la cosecha, toca actualizar el id
            await fertilizacionDao.updateFertilizacion(
              fertilizacion.fertilizacion.copyWith(idFertilizacion: Value(ids[index])),
            );
          }
          index++;
        }
        for (var i in state.fertilizacionesConDiariasPendientes!) {
          await fertilizacionDao.updateSyncFertilizacion(i.fertilizacion, i.fertilizacionesDiarias);
        }
  ...
  Future<bool> syncPodas() async {
    try {
      ...
      final res = await remote.syncPodasConDiarias(state.podasConDiariasPendientes ?? []);
      if (res["success"]) {
        final PodaDao podaDao = db.podaDao;
        var index = 0;
        ...
        for (var poda in state.podasConDiariasPendientes!) {
          if (ids[index] > -1) {
            ...
            await podaDao.updatePoda(poda.poda.copyWith(idPoda: Value(ids[index])));
          }
          index++;
        }
    ...
    try {
      ...
      if (res["success"]) {
        ...
        for (var i in state.palmasPendientes!) {
          await palmasDao.updatePalma(i.copyWith(idPalma: Value(ids[index]), sincronizado: true));
          index++;
        }
  ...
  Future<bool> syncEnfermedades() async {
    try {
      ...
      final PalmaDao palmasDao = db.palmaDao;
      final RegistroEnfermedadDao registroEnfermedadDao = db.registroEnfermedadDao;
      List<RegistroEnfermedadConImagenes> nuevosRegistrosDeEnfermedades = [];
      for (var registro in state.enfermedadesPendientes!) {
        final palma = await palmasDao.getPalma(registro.registroEnfermedad.idPalma);
        if (palma != null) {
          await registroEnfermedadDao.updateRegistro(
            registro.registroEnfermedad.copyWith(idPalmaFromServer: Value(palma.idPalma!)),
          );
          nuevosRegistrosDeEnfermedades.add(
            RegistroEnfermedadConImagenes(
              imagenes: registro.imagenes,
              registroEnfermedad: registro.registroEnfermedad.copyWith(idPalmaFromServer: Value(palma.idPalma!)),
            ),
          );
        }
      }
      ...
      if (res["success"]) {
        ...
        for (var registro in state.enfermedadesPendientes!) {
          if (ids[index] > -1) {
            ...
            await registroEnfermedadDao.updateRegistro(
              registro.registroEnfermedad.copyWith(idPalmaFromServer: Value(ids[index])),
            );
          }
          index++;
        }
        for (var i in state.enfermedadesPendientes!) {
          await registroEnfermedadDao.updateSyncRegistro(i.registroEnfermedad, i.imagenes);
        }
  ...
  Future<bool> syncTratamientos() async {
    try {
      ...
      for (var tratamiento in state.tratamientosPendientes!) {
        final registroEnfermedad = await registroEnfermedadDao.getLocalRegistroEnfermedad(
          tratamiento.idRegistroEnfermedad,
        );
        if (registroEnfermedad != null) {
          if (registroEnfermedad.idRegistroEnfermedad != null) {
            await palmaDao.updateRegistroTratamiento(
              tratamiento.copyWith(idRegistroEnfermedadFromServer: Value(registroEnfermedad.idRegistroEnfermedad)),
            );
            nuevosTratamientos.add(
              tratamiento.copyWith(idRegistroEnfermedadFromServer: Value(registroEnfermedad.idRegistroEnfermedad)),
            );
          }
        }
    ...
    try {
      ...
      if (res["success"]) {
        ...
        for (var registro in state.censosPendientes!) {
          if (ids[index] > -1) {
            //si agrego el registro, toca actualizar el id
            await plagasDao.updateCenso(registro.censo.copyWith(idCenso: Value(ids[index])));
          }
          index++;
        }
  ...
  Future<bool> syncFumigaciones() async {
    try {
      ...
      for (var fumigacion in state.fumigacionesPendientes!) {
        final registroCenso = await plagasDao.getRegistroCenso(fumigacion.idCenso);
        if (registroCenso != null) {
          censos.add(registroCenso);
          if (registroCenso.idCenso != null) {
            nuevasAplicaciones.add(fumigacion.copyWith(idCensoFromServer: Value(registroCenso.idCenso)));
          }
        }
      }
