...
class _FincaPageState extends State<FincaPage> {
  final DateTime fecha = DateTime(DateTime.now().year, DateTime.now().month, DateTime.now().day);
    ...
    return BlocListener<AuthenticationCubit, AuthenticationState>(
      ...
      listener: (context, state) async {
        if (state.status == AuthenticationStatus.unauthenticated) {
          Navigator.of(context).pushNamedAndRemoveUntil('/auth-options', (_) => false);
        }
      },
      child: Scaffold(
        drawer: const SideDrawer(),
        body: Column(children: <Widget>[buildTitulo(), _buildMenu(context)]),
      ),
    );
  }
    ...
    return Padding(
      ...
      child: SizedBox(
        ...
        child: Column(
          children: [
            const SizedBox(height: 20),
            _crearBotonRedondeado2('Ver lotes', '/lotes', FontAwesomeIcons.map, context),
            _crearBotonRedondeado2('Pluviometro', '/pluviometro', FontAwesomeIcons.cloudRain, context),
            _crearBotonRedondeado2('Viajes de fruto', '/viajes', FontAwesomeIcons.truck, context),
            _crearBotonRedondeado2('Bajar de la nube', '/actualizar', FontAwesomeIcons.rotate, context),
            _crearBotonRedondeado2('Subir a la nube', '/sinctoserver', FontAwesomeIcons.cloudArrowUp, context),
          ],
        ),
      ),
    );
  ...
  Widget _crearBotonRedondeado2(String opcion, String ruta, IconData icon, BuildContext context) {
          ...
          children: <Widget>[
            Row(
              children: [
                ...
                Text(opcion, style: const TextStyle(color: Colors.black, fontSize: 18.0)),
              ],
            ),
