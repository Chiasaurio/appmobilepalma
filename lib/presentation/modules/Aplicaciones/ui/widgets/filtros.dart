...
class FiltrosPlagas extends StatelessWidget {
  final String nombreLote;
  const FiltrosPlagas({super.key, required this.nombreLote});

  @override
  Widget build(BuildContext context) {
    return SliverPersistentHeader(
      delegate: MySliverPersistentHeaderDelegate(min: 40, max: 60,
          BlocBuilder<CensosCubit, CensosState>(builder: (context, state) {
        return Padding(
          ...
          child: Row(
            ...
            children: [
              GestureDetector(
                onTap: () => BlocProvider.of<CensosCubit>(context)
              ...
              GestureDetector(
                onTap: () => BlocProvider.of<CensosCubit>(context)
                    .obtenerCensosPendientes(
                        nombreLote, EstadosPlaga.pendientePorFumigar),
                child: Container(
                  padding: const EdgeInsets.symmetric(horizontal: 15),
                  height: 30,
                  decoration: BoxDecoration(
                      color: state.filtro == EstadosPlaga.pendientePorFumigar
                          ? Colors.white
                          : Colors.grey[300],
                      boxShadow: [
                        BoxShadow(
                      ...
                      borderRadius: BorderRadius.circular(10)),
                  child: const Center(
                      child: Text(EstadosPlaga.pendientePorFumigar)),
                ),
              ),
              const SizedBox(
                width: 10,
