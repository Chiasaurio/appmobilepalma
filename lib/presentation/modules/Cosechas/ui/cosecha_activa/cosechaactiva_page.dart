...
class _CosechaActivaVistaState extends State<CosechaActivaVista> {
    ...
    return Container(
      ...
      child: Column(
        children: <Widget>[
          ...
          FilledButton.tonal(
            onPressed: () {
              ConfirmacionAlerta('¿Realmente desea finalizar la cosecha?').confirmacionAlerta(context, () {
                final state = BlocProvider.of<CosechaCubit>(context).state;
                BlocProvider.of<CosechaCubit>(context).finalizarCosecha(state.cosecha!, fechasalida);
                recargarLote(context);
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      backgroundColor: kSuccessColor,
                      content: Text('Se finalizó la cosecha correctamente.'),
                    ),
                  );
          ...
          children: <Widget>[
            Row(
              ...
              children: <Widget>[
                Column(
                  children: <Widget>[
                    const Text('Fecha de inicio ', style: TextStyle(color: Colors.grey, fontSize: 16)),
                ...
                Padding(
                  padding: EdgeInsets.fromLTRB(anchoCard * 0.3, 0.0, 0.0, 0.0),
                  child: TextButton(
                    onPressed: () {
            ...
            Padding(
              ...
              child: Column(
                ...
                children: <Widget>[
                  const Text('Total racimos', style: TextStyle(color: Colors.grey, fontSize: 16)),
                  const SizedBox(height: 5.0),
                  Text(cosecha.cantidadRacimos.toString(), style: const TextStyle(color: Colors.black, fontSize: 20)),
                  const Text(
                    'Total kilos',
  ...
  Widget _buildRegistrarCosecha(BuildContext context) {
    return SecondaryButton(
      ...
      press: () {
        Navigator.push(context, MaterialPageRoute(builder: (context) => CosechaDiariaPage(cosecha: cosecha)));
      },
    );
  }
          ...
          child: Column(
            ...
            children: <Widget>[
              ListTile(
                ...
                title: const Text('Ver registros de cosecha', textAlign: TextAlign.center),
                onTap: () => Navigator.pushNamed(context, '/lote/cosechas/registrocosechas'),
              ),
              const Divider(),
              /*  ListTile(
                    contentPadding: const EdgeInsets.all(10.0),
